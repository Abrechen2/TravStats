...
### Dependency overrides that exist for a published advisory

`backend/package.json` pins two transitive packages above what their parent
asks for. They are listed here because an override with no stated reason is
removed by the next person who tidies the file.

| Override | Advisory | Why it is not simply a version bump |
|---|---|---|
| `deepmerge-ts >=8.0.2` | [GHSA-ggr8-5vv4-36mx](https://github.com/advisories/GHSA-ggr8-5vv4-36mx) — stack exhaustion on recursive object graphs | `@prisma/config` pins `7.1.5` exactly, and the fix only exists in 8.x. It uses one export, `deepmerge`, which 8.x still provides with the same dual CJS/ESM entry points |
| `mysql2 >=3.24.4` | [GHSA-3f6p-5ww8-9rcr](https://github.com/advisories/GHSA-3f6p-5ww8-9rcr) (auth-plugin downgrade leaks plaintext credentials), [GHSA-rgwj-5xj2-c3m3](https://github.com/advisories/GHSA-rgwj-5xj2-c3m3) (decompression-bomb DoS) | The Prisma CLI pins `3.15.3` exactly. TravStats never opens a MySQL connection, but the package is in the production tree, so it is in the audit |

Both reach production through the Prisma CLI, which is a `devDependency` here
and still ships: `@prisma/client` declares `prisma` as an optional peer, the
image's entrypoint runs `prisma migrate deploy` at boot, and `prisma.config.ts`
imports `prisma/config`. Removing the CLI from the image is therefore a
restructuring, not a flag — which is why the advisories are answered with
overrides instead.

`npm audit --fix` would answer them by DOWNGRADING to `prisma@6`, which is not
an option: this schema is generated by the Prisma 7 `prisma-client` generator.

**Drop these the day `prisma` depends on fixed versions itself.** Measured
2026-09-20 against `prisma` 7.10.0: `npm audit --omit=dev` in `backend/` went
from 4 high to 0, `prisma migrate deploy` applied all 147 migrations inside the
built image, and `/health` answered 200.
