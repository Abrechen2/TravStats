...
const assert=require('node:assert/strict'),fs=require('node:fs'),path=require('node:path'),http=require('node:http');const backend=path.resolve(__dirname,'../../.tmp/audit-round2-9678e6cd/backend');
...
const express=dep('express'),app=express();app.use(dep('cookie-parser')());app.use('/uploads',require(path.join(backend,'src/routes/uploads.ts')).default);let routedErrors=0;const errorKinds=[];app.use((e,req,res,next)=>{routedErrors++;errorKinds.push({name:e.name,code:e.code,message:e.message,stack:e.stack?.split("\n").slice(0,12)});next(e);});
app.use(require(path.join(backend,'src/middleware/errorHandler.ts')).errorHandler);const report={commit:'9678e6cd',multerVersion:dep('multer/package.json').version};let server,activeRequest;
async function main(){const u=await prisma.user.create({data:{username:'block17-multer-'+Date.now(),passwordHash:'synthetic-unused'}});const cookie=`auth_token=${generateToken(u.id)}`;
 ...
 report.errorKinds=errorKinds;report.routedErrorsAfterAuthControl=routedErrors-1;report.requestBytes=body.length;report.crashReproduced=report.uncaught?.name==='RangeError';report.runtime=process.version;assert.equal(errorKinds.at(-1)?.name,'RangeError');
}
main().catch(e=>{report.error=e.message;process.exitCode=1;}).finally(async()=>{if(server)await new Promise(r=>server.close(r));await prisma.$disconnect();fs.writeFileSync(path.join(__dirname,'block17-multer-probe.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report));});
