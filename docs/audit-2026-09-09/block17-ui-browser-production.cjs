...
const output=path.join(__dirname,'block17-ui-images-production');fs.mkdirSync(output,{recursive:true});
async function main(){const browser=await chromium.launch({headless:true});try{
 const ctx=await browser.newContext({viewport:{width:1440,height:1000},locale:'de-DE',timezoneId:'Europe/Berlin',extraHTTPHeaders:{Origin:'http://127.0.0.1:13017'}});const blocked=new Set();
 ...
 await page.goto('http://127.0.0.1:13019/login');await page.locator('#username').fill('audit-ui-17');await page.locator('#password').fill('Synthetic-UI-Only-2026');await page.locator('button[type=submit]').click();await page.waitForURL('**/dashboard');
 ...
 for(const width of (process.env.AUDIT_UI_SMOKE ? [1440] : [1440,390,320])){await page.setViewportSize({width,height:width===320?568:900});for(let i=0;i<(process.env.AUDIT_UI_SMOKE ? 1 : routes.length);i++){
   const route=routes[i];await page.goto('http://127.0.0.1:13019'+route);await page.locator('nav').first().waitFor({state:'attached',timeout:15000});await page.waitForTimeout(4000);await page.getByText('Systemstatus wird geprüft',{exact:true}).waitFor({state:'hidden',timeout:15000});
   ...
   fs.writeFileSync(path.join(__dirname,'block17-ui-browser-production.json'),JSON.stringify(report,null,2));
 }console.log('UI viewport completed: '+width);}
 report.blockedExternalHosts=[...blocked];await ctx.close();
}finally{await browser.close();}}
main().catch(e=>{report.error=e.message;process.exitCode=1;}).finally(()=>{fs.writeFileSync(path.join(__dirname,'block17-ui-browser-production.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({views:report.views.length,errors:report.errors,error:report.error}));});
