...
| 4 | Nutzerfarben für Domänen (`domainColors`) behalten? | Streichen. | **So wie jetzt:** Beta-Override über den Token-Defaults bleibt, inklusive Farbwähler. Die Token-Defaults sind die Companion-Farben; wer überschreibt, bricht die Legende wissentlich. | **NACHTRAG 09.09.2026 — diese Entscheidung ist umgekehrt worden.** Ziel für 2.7 ist, dass nichts mehr hinter dem Beta-Schalter steht; auf die Rückfrage, ob das auch für `domainColors` gilt, das ja bewusst dort lag, lautete die Antwort „kommt auch raus". Der Farbwähler wird damit eine gewöhnliche Einstellung ohne Plakette. Umgesetzt auf `dev/v2.7`; BRAND.md §3 und travstats.de ziehen nach Frage 5 nach, also nach Block 1. |
