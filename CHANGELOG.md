...
Some of the larger additions ship behind the instance beta switch, which is
off by default: the passport, the places domain and per-domain colours. They
are complete and tested, but had not been through a release review in time for
this one. Tours, device pairing and the Dawarich connection started out behind
that switch too and came out from behind it in the later candidates, once the
conditions their own gates named had come true. The admin panel lists exactly
what the switch turns on, read from the same registry the application itself
checks.

The candidates after rc.27 also rework how countries are counted — by the
evidence that proves a visit, not by where a flight touched down — and carry
the findings of a full audit of the trackers on 4 September: twelve defects a
tester found on the release candidate, three map defects, and nine older fixes
that had shipped without a test.

### Added
- **A country counts by what proves you were there, not by where a flight touched down.** The country count was too high and too low at once, which is why it looked plausible for so long: a four-hour connection counted as a visit, while a country reached by car and slept in for a week did not exist at all, and three countries with completed hotel stays and no flight ever were missing. Every country now carries the strongest evidence for it, in four rungs — *slept* (a completed stay, a house on record, a port call over a night, arrival and departure on different local days), *visited* (a same-day stay or a recorded place), *transited* (driven through, read from location history) and *connection* (changed planes and nothing else). Only the connection rung is left out of the headline by default; a connection country is greyed and kept, never hidden, because the tier is inferred from recorded flights and flights are recorded incompletely. A day trip is no longer a connection: MUC–FCO–FRA is a day in Rome. Cruise port calls and recorded places now prove a country the way a landing does, and a house entered without any stay counts as one night (owner's rule); a booking still ahead and a cancelled stay prove nothing. There is deliberately no hour threshold — six hours and twelve hours returned the same set of countries on a real account, so a dial would promise a precision the data does not hold. Only attested days count: a landing in January and the next departure five years later no longer read as five years of presence. **This moves the headline for every account**: it rises for accounts with cruises, places or hotel stays, and falls for accounts whose countries rested on connections.
- **Every counted country names its evidence, and the record is one click away.** The passport table shows each country's rung, the records that proved it, the days present, and the time on the ground where a pair of flights bounded one. A duration that cannot be derived reads "unknown", never zero, and a country no flight touched shows a dash — the two differ because only the first says "add the missing flight". Expanding a row lists the flights, cruises, places and houses behind it, each linking to the page that edits it. A country page (`GET /stats/countries/:code`) answers for a country reached only by ship or by a recorded place, instead of a 404 behind a clickable row.
- **How strictly countries are counted is a setting — an instance default, and a per-user override.** Whether a connection counts is a personal definition, not a property of the server, so an administrator sets the starting point and anyone may override it. The choice is one of the evidence rungs; there is no hours option on purpose. Changing it moves the headline and nothing else: the country list is identical at every threshold, and the year in review and the achievements read the same threshold rather than counting airside stopovers. Badge progress can drop for an account holding connection-only countries; a badge already earned stays earned.
- **Driving through a country is its own rung, read from location history.** A Dawarich connection is now reduced nightly to country-days — "on this day, in this country" — and folded into the passport as a fifth kind of evidence. Coordinates exist only for the length of one loop and are never stored, returned or logged; what a row keeps is the day, the country and a count of fixes, so a day held up by four hundred points and one held up by a single point can be told apart. A country is a connection when every point ever recorded there lay within an airport's grounds you are known to have flown through; a point beyond that lifts it onto the stronger rung. The road rung and its filter only appear once an account has location history, because a filter that always returns nothing reads as a bug.
- **The passport says once that the counting changed, with your real figures.** A number that moves without explanation reads as data loss, so the page shows "Zählweise geändert — vorher 32 Länder, jetzt 35" the first time the old rule and the new one disagree, computed from the same rows so the two can never drift, with a link to the evidence explanation and a dismiss that is remembered in your browser.
- **Data that contradicts itself is flagged and queued — never rejected, never silently corrected.** Four checks fire only where two sources inside one record disagree: an address whose country differs from the stored one, coordinates that fall outside the claimed country, a country resting solely on records with no date, and a check-out before its check-in. A third-party geocoder does not get a veto over your own data, so the record is written and the question waits. A flag never changes a number by itself. A point in the sea or in a territory the offline outlines do not attribute is not a disagreement. A nightly sweep and every import run the checks.
- **The pending-updates page is now the inbox.** It holds the flight proposals it always had plus the data-quality questions, one list for the user with two tables underneath. Each flag states both conflicting values and marks neither as correct. The two answers are different kinds of statement, and each carries its consequence: "I corrected it" lets a surviving contradiction come back; "This is right" never asks again — the escape hatch for a district that shares a country's name. The nav badge counts both halves independently, so one failing cannot hide the other.
- **A document announces itself.** A pasted mail, a `.msg`/`.eml` file or a PDF can be sent with `domain: "auto"` and the server decides whether it is a flight, a cruise or a hotel — a property of the document, not of the button that sent it. Detection is a weighted scorer rather than the language model, so it works on an instance with no Ollama; a signal counts once however often it repeats, because marketing mail is the longest thing anybody forwards. A weak answer still returns what it parsed, with the runners-up, so a client can offer a switch instead of asking for the file again. An explicitly named domain keeps its exact previous answer (forgejo#57).
- **A photograph is a document.** `POST /parse-image` reads a photographed confirmation — a hotel bill, the most photographed travel document there is — through OCR and hands the text to the same parser as a mail or a PDF. Until now an image could only ever become a flight. The OCR worker reads German as well as English, since the lodging parser matches on "Nächte", and degrades to English on an air-gapped instance rather than failing (forgejo#58).
- **A photographed boarding pass is read by its barcode first.** The parse route decoded only the printed text while the propose route beside it, handed the same picture, decoded the PDF417 or Aztec code and used OCR merely to fill gaps — so a Wallet screenshot could come back "unreadable" with the answer sitting in the middle of the picture. A decoded barcode wins every field it carries; OCR keeps the fields no barcode holds (gate, terminal, boarding group). The airline is now reported by its printed name rather than its two-letter code on both routes (forgejo#37).
- **The travel records, the whole route network and the year in review come from the server.** `GET /stats/records` computes the seven records (longest and shortest leg, longest time aloft, streaks, delays) once, as numbers with units rather than formatted sentences; `GET /stats/network` returns every airport with coordinates and every route with a count and a distance, deliberately unbounded because a truncated network is not a smaller globe but a wrong one; `GET /stats/wrapped?year=` reads the year off the data and reports a favourite the year cannot support as absent rather than zero, and a year with no flights no longer headlines as the biggest flying year of your life (forgejo#41, forgejo#42, forgejo#44).
- **Four more curated checklists, each verified rather than recalled.** The New 7 Wonders of Nature, the UNESCO Biosphere Reserves (646 of ~759 — the register entries with no coordinate are dropped rather than pinned at 0/0, and the gap is stated), and the national parks of Germany (16) and the United States (63). Two coordinates guessed from memory turned out to be an Italian village and an American singer, so every row was checked; a natural wonder gets an arrival point, never the centroid of a rainforest (forgejo#66). Alongside them, fourteen museum warships — USS Missouri, HMS Victory, Mikasa, Vasa, U 995 — with two collector achievements.
- **A geotagged photo suggests the place it was taken at.** A trip with a linked Immich album is already a list of dated, positioned points, and nothing was reading them. A photo is the only anchor that is not a proxy — a hotel is booked in the town you came to see, but a photograph is a GPS fix at the moment the shutter opened — so its radius is one kilometre and it outranks a hotel or a logged place as the stated reason.
- **Places can be imported from a CSV or a Google Takeout list, and the rows it cannot place come back to you.** German decimals are read as such ("41,8902"), and a row with no coordinates — every row of a Takeout export — is neither dropped nor written: it returns as *needs input* with your note intact, for one click in the picker. A place added by hand now carries the same identity a later import would mint, so the Colosseum picked today and imported next month is one place, not two. The batch appears in the import log and can be undone. The web import panel for places stays behind the beta switch with the rest of the domain; the routes are live for the API and the Companion.
- **Several flight proposals can be answered in one request.** `POST /pending-updates/apply` and `/reject` take a list of ids and report an outcome per id — a flight edited meanwhile, one already applied, one that is not yours — rather than one success flag that would silently drop changes you believe you accepted (forgejo#33).
- **The backlog refresh says which kind of nothing happened.** "The provider has nothing on this leg" and "the provider answered and your record was already complete" shared one counter, so a run that had just written three tail numbers read as a run that changed nothing. *Already complete* is its own line now, and every silent per-flight result carries the provider's own reason — no key, a date the free tier refuses, or a leg it genuinely does not know.
- **A flight that landed without its actual times is asked about one last time.** For an overnight flight both arrival checks fell on a date the free plan refuses, and once the sweep marked it flown nothing ever asked again. The sweep now leaves one late check outstanding and spends it through the one provider that answers for a past date — exactly once, values filled and never overwritten, and only for flights the sweep has just flipped, so the cost is the number of flights you actually took, not the size of your logbook.
...
### Changed
- **Tours, the Companion app pairing and the Dawarich connection stay behind the beta switch.** All three work end to end and were briefly offered to everyone in a release candidate; the owner ruled them unfinished. An instance with the switch off does not offer the Tours tab, the trip route editor, the Devices section that mints a claim code, or the Dawarich card. Each gate names why it is still there and what brings it back; the places domain stays gated as well (forgejo#65, forgejo#73).
- **A route is the pair, not the direction.** FRA→WAW and WAW→FRA were two routes with one flight each while the Companion's globe counted one route with two. Both legs now land on one entry, keyed by the two codes sorted, so an existing account's top-routes list reorders: two entries of one collapse into one of two (forgejo#42).
- **A flight's duration is owned by the database and cannot go stale.** It was derived independently in three server places and again in the app, and eight code paths write the timestamps it depends on. It is now a generated column that Postgres refuses to hand-write, so no present or future write path can leave an old number behind. Rows whose duration depends on the airport catalogue are deliberately not stored, so a timezone correction lands on the next read. On the way, `GET /flights/:id` and the create and update responses stopped returning a bare row that could show a null duration where the list showed a number (forgejo#45).
- **The flight table is indexed for the shape the statistics actually ask.** Two composite indexes replace ten single-column ones for the hot queries; on a multi-user instance the counting queries stop reading other users' rows (measured 5007 buffers down to about 20). On a single-account instance this buys nothing and costs nothing — the planner ignores an index that does not help (forgejo#48).
- **"Transit" is renamed "connection" wherever a country's evidence is shown**, in stored settings as well as on screen, because *Transit* is the word a German reader attaches to driving through the moment a road rung exists. A stored threshold of "visited" becomes "transited" by the same reasoning, so no instance's headline moves on the rename alone.
...
- **Two beta gates closed properly.** With the instance's beta switch off, the
  ...
  answered with "enable the flights domain" to a reader whose flights were on.
  The tab strip and the overview follow the same rule as the deep link now,
  and a closed passport gate goes home like /places.
...
- **A flight number is not unique within a day — the airport decides.** LX93 on 2026-09-02 came back three times: the long-haul that left the day before, a feeder that afternoon, and the one actually flown. The departure airport now narrows the candidates, as a preference rather than a filter, so an older row with no code is no worse off. The lookup cache key carries the airport too, because two flights sharing a number on one day are different answers and the old key would have served the first caller's aeroplane to the second.
- **A provider may not rename the carrier you flew with.** LX1104 Zurich–Munich, operated by Helvetic and stored as such, was rewritten to "United Airlines" by an auto-applied update while the codes beside it still said Helvetic — on a codeshare the marketing and the operating carrier are two true answers and the provider may report either. Your own text now wins; an empty column is still filled. It also ends the "SWISS" against "Swiss" proposal on every refresh.
- **The lodging import finds a house under a decorated name.** "Hotel Meteora" beside "Hotel Restaurant Meteora", "Krafft Basel" beside "Hotel Krafft Basel" — five pairs on one account, because the matcher keyed on the exact name and the coordinate rule behind it never fired: a booking mail and a saved-places export both arrive without a pin. Two names are now offered as one house when the town agrees and they share an identifying word; a hit is a guess you confirm, never a silent merge, and "Hotel Rose" in Portland and in Bietigheim stay two houses (forgejo#84).
- **The postcode stays out of the city, from both parsers.** "188973 Singapur" and "BW 78467 Konstanz" sat in the city column: the template admitted four or five digits and nothing in front, the language model copied the city as printed, and the geocoder never repairs a city that is wrong rather than empty. One splitter now takes a leading code off the city — six digits, a state abbreviation in front, Dutch letters, a Luxembourg L- prefix — and both parsers use it (forgejo#85).
- **The same airline is the same code, on every surface.** One account counted 31, 27, 26 and 25 airlines on four screens, because identity was the name: "SWISS" and "Swiss" were two carriers, "LOT" and "LOT - Polish Airlines" two more, and four flights with no airline formed a group labelled "". Two rows are now the same airline when they share an IATA code, resolved through the catalogue; a row with no airline is counted apart and the breakdown says how many it left out (forgejo#81).
- **"Länder besucht" for a single year no longer shows the lifetime set.** The lodging domain had no per-year country index, so the overview fell back to every country ever slept in under a "2024" header — 35 where 15 were flown and 7 slept in that year. Stays are keyed by their check-in year now, like flights. A house entered without any stay still counts as its country; that is the owner's rule from the country-counting design, and the issue's second half stands the other way (forgejo#80).
- **A planned, priced stay no longer reports its hotel as "0 €".** The spend cell chose its branch from all stays but summed only the ones that count, so a hotel not yet slept in read as free. Nothing counted reads "—", and a priced stay still ahead shows a muted "geplant: 149,90 €" beneath it (forgejo#82).
- **"Add POI" on the map opens the place form.** The map's "+" menu offered it whenever the domain was switched on, and clicking it did nothing — the slot held a comment from before the domain existed. The form now opens and stays on the map after a save, so you see the pin; the menu entry follows the same visibility rule as the tab strip (#288).
- **One formatter for every amount — no more "EUR 40206" beside "40.206 €".** The same trip total was written three ways across the trip page, the cruise page and the trip card; ten sites carried their own spelling, and three of them defaulted a missing currency to EUR, turning "unknown" into a claim. Every amount now goes through one formatter, and a missing currency yields a bare number rather than an invented euro (forgejo#86).
- **The continents tile counts against the one shared table.** Statistiken → Flüge said "Kontinente 6/ 6" while its own caption said "of the 7" and the passport said 6/7 — three homes for one number, including a private six-bucket table with no Antarctica that filed McMurdo and Bermuda under "Other". Every airport now resolves through the same continent table the passport uses, and the denominator travels with the response (forgejo#87).
- **Modal backdrops dim the page again instead of painting it black.** The add-flight form opened from the map sat on a solid black page. Its backdrop used Tailwind 3 spelling for the 50 % opacity, which Tailwind 4 drops without an error — thirteen hand-rolled backdrops had the dead class; a guard now scans for it (#289).
- **The map camera stays where you left it across a domain switch.** Every dashboard tab owns its own map, and each handed MapLibre the same fixed starting view, so a user zoomed into Berlin on the flight tab landed back at zoom 2 on the cruise tab, every time — and after every save. The last camera is remembered for the session, per engine, since the flat map and the globe use different zoom scales. Deliberately not persisted across a reload; that is a product decision not yet taken (#290).
- **A year with no priced flight has no cost total, not "0 €".** The cost rule started at zero and returned zero when nothing was added, a claim that the flights were free where the truth is that nothing was recorded. The tile now shows a dash with "No price recorded — 3 flights without one", and the trend pill appears only when both years have a number (forgejo#83).
- **The backup routes answer with the honest status, not 500.** "That backup does not exist", "that backup is not finished" and "WebDAV was never switched on" all arrived as a server fault, filled the error log with client conditions, and the documented 404 could never be produced. They are 404, 400 and 409 now; only a share that answered badly is a 502, with the share's own words kept (forgejo#77).
- **The address backfill bounds the work, not the scan.** Both nightly passes — places with a pin and no description, lodgings with a pin and no address — read a user's first 500 rows and filtered those, so an account with more complete rows than that could never get an address for anything newer; the bound had become "the first page, forever". The scan now walks in cursor pages until the candidates are in hand (forgejo#43).
- **A pin in no settlement still names the place it lies in.** Five landmarks — national parks and a bay — read "—" in the place column, because the reverse geocoder consulted only city, town, village and municipality, and wilderness has none. The chain continues into hamlet, suburb, district, county and state, smallest first; the district or the state beats nothing. Rows already empty fill on the next nightly run.
- **A hotel-only trip shows the country of its stays.** A trip with linked stays and no flight or cruise read "?" for its countries, because the derivation knew only airports and ports. A linked stay contributes its country the way a linked cruise does; a cancelled booking never put anyone anywhere and is skipped.
- **The trend chart reads the airport clock, like everything else.** It bucketed by the stored UTC instant while every other "when did I fly" figure reads the clock at the departure airport, so a flight leaving Bangkok at 06:00 on 1 January sat in the old year on the chart and in the new year in the country list. The fetch window is widened a day at each edge so the flight is found at all, and one filter decides what is in the window for the series and the totals alike (forgejo#46).
- **A cancelled house is not a visited one, and an unlock date means an unlock.** A house whose stays were all cancelled still read as visited and still proved a country to the achievements. And 103 badges stood "unlocked below their requirement" because the unlock date defaulted to the row's creation — a badge at 86 of 100 carried the day tracking began, and COUNTRIES_100 appeared to predate COUNTRIES_50. Nothing on screen was wrong; the column was. The date is now set only on a genuine unlock, cleared on revocation, and the migration nulls every row sitting below its requirement.
- **A bookmarked house proves no country, so its flag goes too.** Un-marking a hotel as visited removed the country from the passport immediately while the inbox still asked about a country that no longer existed — the same two-places-two-answers fault this rework exists to remove, reappearing inside the feature built to catch it. A house's own address is still checked whether or not anybody slept there.
- **The hero tile counts countries by the rule, like everything else.** Five consumers moved onto the shared evidence rule; the hero tile — which backs the Companion's start board — kept reading "countries a flight touched", so one account answered "how many countries" twice, a swipe apart, with different numbers. It was also the very number the owner was looking at when he reported the count as too high.
- **Achievements no longer join country names where the passport joins codes.** One country counted several times — 88 against the passport's 32, from 56 distinct spellings for 33 codes ("Deutschland" and "Germany", "Cesko" and "Tschechien"). The name-to-code join now exists once and every country list resolves through it; a two-letter Japanese place name is no longer published as a country code.
- **The parser reads the card, not the margin around it.** A Wallet screenshot yielded the status-bar clock, the word "Lufthansa" and the security number — no flight, no route, no date. Tesseract picks one global threshold for the whole image, which a navy-on-orange card in a black letterbox defeats twice. Thresholding per neighbourhood takes the twelve sample passes from 16 correct fields to 37 with fewer wrong ones. Reading more of the card also read the barcode as rubble, so printed city names now win over stray three-letter fragments, and lines of one- and two-letter debris are dropped before airport codes are harvested (forgejo#36, forgejo#37).
- **The pass says which flight, not which column.** An ordinary German boarding pass came back as GRP → MUC on no date, with the boarding time for a gate and "KLASSE" for a booking reference: the date pattern demanded a clock beside it, the route took the first two three-letter tokens in reading order — which are the column headings — "Boarding" was read as a gate label, and any six-letter word was a booking reference. That last one merged a scan into an unrelated flight, since a PNR is looked up before the flight number. Known airports are preferred over reading order, a date may stand without a time, and an unlabelled reference now needs a digit (forgejo#36).
- **A gate is not a flight, however OCR spells it.** Gates print as "B08" and "E06", and O-for-0 turns them into "BO8" and "EO6" — exactly the shape of a flight number, printed above the real one. Three of twelve sample passes came back as flights EO6, KII8 and BO8. Every candidate is collected and the first with a real carrier prefix wins; an unknown prefix is still accepted when nothing better is on the pass (forgejo#36).
- **"ab 380 EUR" is a price, not flight AB380.** Six of eight archived Emirates promotions came back as a flight. The guard for it existed from the start and never fired: the candidate reached it in lower case, and the check sliced two letters, so "Nur 7 Tage gültig" passed as NUR7. Compared upper-cased on the whole alphabetic prefix; all eight promotions now yield nothing and every real booking still resolves (forgejo#35).
- **The flight wins over the weekday and the fare beside it.** A genuine Emirates confirmation produced an invented MO24 and lost its real EK0050: the context lookahead did not cross a line break, the flight number sits alone on its line, and the line below — weekday, date, "Flughafen" — contained the word the rule wanted. Every match of every pattern is collected and one whose prefix the airline catalogue knows is preferred. Measured over 337 archived mails: EK0050, UA384, LH2040, AF1423 and others recovered; DL3, BER36, PRO1 and other phantoms dropped (#291).
- **The evidence rule guards every provider, not just the fallback.** The rule that a booking must carry a flight number or both ends of a route — a date is not evidence, every marketing mail has one — lived in the template parser, which runs only when the language model is unavailable. On every instance with an LLM configured the guarded path was the one that did not run, and a "30 EUR Oster-Geschenk" promotion came back as three flights with every field empty. The rule now applies to whatever the chosen provider returned (forgejo#35).
- **A mail with no booking is an answer, not a server fault.** A provider that read the mail and found nothing was filed as a provider failure; with the chain exhausted the request became HTTP 500, and the import modal sent the user looking for a broken instance. Finding nothing now returns an empty result; only a chain in which no parser ran at all still fails (forgejo#35).
- **The route a mail prints in brackets is recovered.** An Air France confirmation writes each leg as "München (MUC), DEUTSCHLAND" and the model returned the legs without codes; nothing put them back although the text held them. The rule is structural — a three-letter code in brackets — because the airport catalogue contains DIE, DER, DAS, UND and BEI as airports somewhere, and the hand-kept safe list lacked Nantes, the very leg reported lost. Pairing is all-or-nothing, so a stray bracketed word leaves the route empty rather than guessed (forgejo#35).
- **The hotel parser is shown the booking, not the link farm.** A 2009 hotels.com confirmation came back "no booking found", and rewriting its mixed German/English dates changed nothing. Two thirds of the mail was tracking URLs; with links replaced by placeholders the whole document parsed on the first try. The cleaning the flight parser has applied all along now runs on the lodging side too — for the model only, since a template may key on a link (forgejo#34).
- **The hotel parser says who looked and what was dropped.** "No parser produced a result" read as "no parser ran", which is how a working instance came to be reported as having none; a truncated document was cut silently; and a model answer missing one required field was discarded without a word. Each is now named in the result or the log. The rule itself — a name and both dates — is unchanged (forgejo#34).
- **Saving no longer collides with a badge re-check.** Every achievement was re-evaluated inside one long transaction that most call sites did not await, so two runs for one user overlapped — a flight saved while a place tick was still going — and deadlocked in Postgres, or wrote to rows deleted underneath them. The run now decides first without touching the database and opens a transaction only when there is something to write; runs for one user are chained, and the call sites wait for them, so the work cannot outlive the request (forgejo#39).
...
### Security
- **The page carries a Content-Security-Policy.** Helmet set one, but only on `/api` responses, which govern nothing that matters for script injection — the HTML that decides whether injected script may run was served with no CSP at all. `script-src 'self'` with no `unsafe-eval`, measured against the production bundle rather than hoped for; `connect-src` stays deliberately broad because a self-hosted instance talks to the Immich, Dawarich or OSRM the operator names, often over plain http on a LAN. Verified in a browser — the first attempt silently blocked the fonts and Leaflet's stylesheet, which no reading of the header would have shown (forgejo#29).
- **A deactivated account is refused a session, on every path.** Login answered a disabled account with HTTP 200 and a fresh cookie; the lock held on the next request, but the door still handed out a key. Five routes mint a session — login, registration, first-boot setup, two-factor verification and passkey assertion — and all five now go through one function that refuses an inactive subject. The two-factor branch is guarded separately, so a disabled account no longer receives a "password accepted, now finish signing in" (forgejo#31).
- **Every unlimited route now carries a verdict.** The rule "rate limiting on every expensive endpoint" was practised but unreadable: a reader could not tell "considered" from "forgotten". Twenty-five routers now carry a limiter or one sentence saying why they need none. Limited on the way, by cost: the Immich photo-journey scan (the most expensive request in the app), CSV preview and parse, training and photo uploads, the cruise geometry batch, the achievements leaderboard — the one route whose cost scales with the instance rather than the caller — and the checklist suggestions (forgejo#51).
- **Rate limits count per user, not per address.** The flight-lookup, analytics, PDF-parse and boarding-pass limiters sat behind authentication but were keyed by IP, so a household — or every user behind one reverse proxy — shared a single bucket and one person's lookups blocked everyone on the same connection. A guard now demands the per-user key on every limiter that runs after authentication (forgejo#68).
- **Refreshing the airline templates is an admin action.** Any signed-in account could replace the template registry every user of the instance parses with, and spend the instance's three GitHub refreshes per hour. The status stays readable by everyone — which airlines the mail parser understands is a user question — and the page explains why the button is missing instead of offering one that always fails (forgejo#67).
