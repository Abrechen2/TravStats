...
### Added
- **Auto-backup before major-version migrations** — On boot, the entrypoint now reads `/app/data/backups/last-version`, compares the major number to the current `APP_VERSION`, and if it has bumped (e.g. 1.x → 2.x) snapshots the database to `/app/data/backups/pre-vX.Y.Z-upgrade-<timestamp>.sql.gz` BEFORE `prisma migrate deploy` runs. After a green migration the new version is persisted, so subsequent boots only back up on real bumps. Backup failures are deliberately soft — they log a warning and let the migration proceed, so a missing Docker socket or full disk doesn't paint users into a corner.
