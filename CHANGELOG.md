...
### Changed
- **PostGIS extension is no longer declared at the schema layer** — `extensions = [postgis]` and the `postgresqlExtensions` preview feature were removed from `schema.prisma`, and the `CREATE EXTENSION IF NOT EXISTS "postgis"` line was dropped from migration `20260430120000_post_v2_drift_fix`. No column ever used PostGIS geometry types — cruise-route geometries are computed in TypeScript and stored as JSON — so the dependency was latent infra coupling. Existing installs are unaffected (the extension stays installed where it was created by the init migration). When real spatial features land, a dedicated migration will reintroduce PostGIS with a pre-flight check.

### Added
- **Auto-backup before major-version migrations** — On boot, the entrypoint now reads `/app/data/backups/last-version`, compares the major number to the current `APP_VERSION`, and if it has bumped (e.g. 1.x → 2.x) snapshots the database to `/app/data/backups/pre-vX.Y.Z-upgrade-<timestamp>.sql` BEFORE `prisma migrate deploy` runs. The hook is wired straight into `docker-entrypoint.sh` (production never calls `init.ts`). After a green migration the new version is persisted to `/app/data/backups/last-version`, so subsequent boots only back up on real bumps. Backup failures are deliberately soft — they log a warning and let the migration proceed, so a missing Docker socket or full disk doesn't paint users into a corner.
