...
export function calculateBusinessStats(flights: FlightData[]): BusinessStats {
  ...
  const costPerKm = totalDistance > 0 && totalCost > 0 
  ...
  const costPerHour = totalFlightHours > 0 && totalCostForHours > 0
    ? Math.round((totalCostForHours / totalFlightHours) * 100) / 100
    : 0;
