 ...
 */
export function getContinent(lat: number, lon: number, country?: string | null): Continent | null {
...
export function countryCountsByContinent(): Record<Continent, number> {
  const totals = Object.fromEntries(CONTINENTS.map((c) => [c, 0])) as Record<Continent, number>;
