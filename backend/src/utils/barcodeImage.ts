/**
 * Decode an Aztec / PDF417 boarding-pass barcode directly FROM an image.
 *
 ...
 * null if no Aztec/PDF417 barcode could be read. Never throws.
 */
export async function decodeBarcodeFromImageBase64(imageBase64: string): Promise<string | null> {
  try {
    ...
    const results = await readBarcodes(blob, { formats: ["Aztec", "PDF417"], tryHarder: true });
