...
describe("calculateLodgingStats", () => {
  ...
  it("counts a lodging with zero stays as a house AND as its country — owner's decision", () => {
    // 2026-08-15: a hotel entered by hand without a stay still counts as a
    // lodging. 2026-09-02 (country-counting design §1.4): it counts as one
    // night, so its country counts too — somebody took the trouble to enter
    // the house, so they were there. forgejo#80 asked for the opposite; the
    // owner's decision stands, and the per-year index below is the part of
    // that issue that was a real defect.
    const lodgings = [
