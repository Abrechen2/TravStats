...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const retainInput = {
        buffer: Buffer.from(validatedImage, "base64"),
      ...
      if (
        ...
        try {
          ...
          const { flights: apiFlights } = await lookupFlightWithHistorical(
            flight.flightNumber,
            date
          );
          ...
          logger.warn(
            { error: apiError },
            "[Boarding Pass Parse] Flight lookup API failed, continuing without enrichment"
          );
        }
      }
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const tesseractAvailable =
        providers.vision.find((p) => p.provider === "tesseract")?.availability.available || false;
