...
const enrichAirportSchema = z
  ...
  .refine((data) => data.iata || data.icao || (data.lat !== undefined && data.lon !== undefined), {
...
router.get(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const maxDistance = req.query.maxDistance ? parseFloat(req.query.maxDistance as string) : 5;
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (data.iata) {
        const clash = await prisma.airport.findFirst({
          where: { iata: data.iata, isClosed: false },
        });
        if (clash)
          throw new AppError(`An active airport with IATA ${data.iata} already exists`, 409);
      }
      if (data.icao) {
        const clash = await prisma.airport.findFirst({
          where: { icao: data.icao, isClosed: false },
        });
        if (clash)
          throw new AppError(`An active airport with ICAO ${data.icao} already exists`, 409);
      }
      ...
      logger.info({
        operation: "airport_create_manual",
