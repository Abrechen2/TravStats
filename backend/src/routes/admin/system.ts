...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const { invalidateAirlineCatalogCache, preloadAirlineCatalog } =
        await import("../../services/airlineCatalogCache");
      const inserted = await seedAirlinesFromData();
      invalidateAirlineCatalogCache();
      await preloadAirlineCatalog();
      logger.info({
        operation: "admin_airline_reseed",
        context: { inserted, triggeredBy: req.userId },
      });
      res.json({ inserted });
    } catch (error) {
      next(error);
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const { invalidateAircraftCatalogCache, preloadAircraftCatalog } =
        await import("../../services/aircraftCatalogCache");
      const inserted = await seedAircraftFromData();
      invalidateAircraftCatalogCache();
      await preloadAircraftCatalog();
      logger.info({
        operation: "admin_aircraft_reseed",
        context: { inserted, triggeredBy: req.userId },
      });
      res.json({ inserted });
    } catch (error) {
...
router.get(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename*=UTF-8''${encodeURIComponent(exportFilename)}`
      );
      res.json(exportData);
    } catch (error) {
      next(error);
