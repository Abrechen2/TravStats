...
describe("Cruises API", () => {
  ...
  beforeAll(async () => {
    await prisma.cruise.deleteMany({
      where: { user: { username: { in: ["cruisetest", "cruiseother"] } } },
    });
    await prisma.user.deleteMany({ where: { username: { in: ["cruisetest", "cruiseother"] } } });

    const u = await prisma.user.create({
      data: { username: "cruisetest", passwordHash: await hashPassword("password123") },
    });
    userId = u.id;
    authCookie = `auth_token=${generateToken(u.id)}`;

    const other = await prisma.user.create({
      data: { username: "cruiseother", passwordHash: await hashPassword("password123") },
    });
  ...
  describe("GET /api/v1/cruises", () => {
    ...
    it("filters by cruiseLine", async () => {
      const res = await request(app)
        .get("/api/v1/cruises?cruiseLine=AIDA%20Cruises")
  ...
  describe("GET /api/v1/cruises/:id", () => {
    ...
    it("returns 404 for another user's cruise (no data leak)", async () => {
      const foreign = await prisma.cruise.create({
        data: { userId: otherUserId, status: "scheduled" },
      });
  ...
  describe("POST /api/v1/cruises", () => {
    ...
    it("persists and returns the optional map color (round-trip)", async () => {
      const create = await request(app).post("/api/v1/cruises").set("Cookie", authCookie).send({
  ...
  describe("DELETE /api/v1/cruises/:id", () => {
    ...
    it("404 on another user's cruise", async () => {
      ...
      const res = await request(app)
        .delete(`/api/v1/cruises/${foreign.id}`)
        .set("Cookie", authCookie);
      expect(res.status).toBe(404);
