...
describe("PATCH /api/v1/photo-journeys/:id", () => {
  ...
  beforeAll(async () => {
    ...
    userId = (await prisma.user.create({ data: { username: `pj-patch-${stamp}`, passwordHash } }))
      .id;
    strangerId = (
      await prisma.user.create({ data: { username: `pj-patch-other-${stamp}`, passwordHash } })
    ).id;
  ...
  it("records the visit a place finding was accepted with", async () => {
    ...
    const place = await prisma.place.create({
      data: { userId, name: "Café Norte", lat: 38.72, lon: -9.14 },
    });
  ...
  it("refuses to link another user's trip, and leaves the row unanswered", async () => {
    ...
    const foreignTrip = await prisma.trip.create({
      data: { userId: strangerId, name: "Not yours" },
    });
