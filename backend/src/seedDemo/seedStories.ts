...
async function seedStory(
  ...
  const trip = await prisma.trip.create({
    data: {
      ...
      startDate: new Date(story.start),
      endDate: new Date(story.end),
      originLabel: story.origin,
      destinationLabel: story.destination,
  ...
  if (companions.length > 0) {
    await prisma.tripCompanion.createMany({
      data: linkRowsFor(companions.map((c) => c.id)).map((l) => ({
        tripId: trip.id,
  ...
  for (const f of story.flights) {
    ...
    const flight = await prisma.flight.create({
      data: {
        ...
        status: planned ? "scheduled" : "flown",
        seatClass: "economy",
        category: "vacation",
        companions: story.companions,
        tags: story.tags,
  ...
  for (const s of story.stays) {
    ...
    await prisma.lodgingStay.create({
      data: {
        ...
        board: s.board,
        guests: 1 + story.companions.length,
        currency: s.currency,
        totalPrice: s.price,
        ...seedFxColumns(
          { totalPrice: s.price, currency: s.currency, checkIn: new Date(s.checkIn) },
          baseCurrency
        ),
        ratingOverall: s.rating,
        companions: story.companions,
        dataSource: "manual",
      },
    });
  ...
  for (const [orderIdx, p] of story.places.entries()) {
    ...
    const place = await prisma.place.create({
      data: {
        ...
        city: p.city,
        country: p.country,
        isoCountryCode: p.iso,
        visited: p.visitedAt !== null,
        curatedItemId,
  ...
  if (story.cruise) {
    ...
    for (const stop of c.stops)
      portIds.push("locode" in stop ? await portIdByLocode(stop.locode) : null);
    ...
    const cruise = await prisma.cruise.create({
      data: {
        ...
        departurePortId: firstPort,
        arrivalPortId: lastPort,
