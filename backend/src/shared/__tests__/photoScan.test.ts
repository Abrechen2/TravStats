...
const FLIGHTS: FlightEndpoints[] = [
  { depIata: "MUC", depLat: MUC.lat, depLon: MUC.lon, arrIata: "CAI", arrLat: CAI.lat, arrLon: CAI.lon },
  { depIata: "CAI", depLat: CAI.lat, depLon: CAI.lon, arrIata: "MUC", arrLat: MUC.lat, arrLon: MUC.lon },
  { depIata: "MUC", depLat: MUC.lat, depLon: MUC.lon, arrIata: "LIS", arrLat: LISBON.lat, arrLon: LISBON.lon },
];

const burst = (samples: { lat: number; lon: number }[], days = 0, startMs = MAY_1): LocatedCluster => ({
  startMs,
...
describe("photoScan", () => {
  ...
  it("counts arrivals as well as departures towards home", () => {
    ...
    expect(mostVisitedIata([{ arrIata: "LIS" }, { arrIata: "LIS" }, { depIata: "MUC" }])).toBe("LIS");
  });
