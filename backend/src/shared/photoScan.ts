...
export function homeCoordinate(
...
export function placeHunches(
  ...
  for (const cluster of clusters) {
    ...
    for (const place of places) {
      ...
      const known = place.visits.some(
        (visit) => visit.visitedAt != null && days.includes(isoDay(visit.visitedAt))
      );
      if (known) continue;
...
function spreadAwayFromHome(coords: readonly Coordinate[], home: Coordinate | null): number {
  ...
  const away =
    home === null ? coords : coords.filter((at) => haversineKm(home, at) > PHOTO_SCAN_HOME_NEAR_KM);
