...
async function main() {
  ...
  for (let i = 0; i < totalFlights; i++) {
    ...
    const distance =
      Math.sqrt(
        Math.pow(arrAirport.lat - depAirport.lat, 2) + Math.pow(arrAirport.lon - depAirport.lon, 2)
    ...
    if (rand < 0.7) seatClass = "economy";
    else if (rand < 0.9) seatClass = "business";
    ...
    flights.push({
      ...
      notes:
        Math.random() > 0.8
          ? `${seatClass === "business" || seatClass === "first" ? "Excellent service!" : "Good flight"} on ${aircraft}`
          : null,
    });
  }
