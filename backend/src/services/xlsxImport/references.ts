...
async function ownsParent(kind: ParentKind, id: string, userId: string): Promise<boolean> {
  ...
  if (kind === "roadtrip" || kind === "tour") {
    return Boolean(await prisma.tripRoute.findFirst({ where: { ...where, kind }, select }));
  }
...
async function storedLabels(kind: ParentKind, userId: string): Promise<Map<string, Set<string>>> {
  ...
  if (kind === "roadtrip" || kind === "tour") {
    // The export writes a route's plain name as the readable half — see
    // `frontend/src/lib/xlsx/roadtripSheets.ts`.
    const rows = await prisma.tripRoute.findMany({
      where: { userId, kind },
      select: { id: true, name: true },
    });
    for (const r of rows) add(r.id, labelForms(r.name));
  } else if (kind === "place") {
