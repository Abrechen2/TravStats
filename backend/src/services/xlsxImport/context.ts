...
/** A sheet whose rows other sheets point at ("Name [id]"). */
export type ParentKind = "cruise" | "lodging" | "place" | "roadtrip" | "tour";

const PARENT_KINDS: readonly ParentKind[] = ["cruise", "lodging", "place", "roadtrip", "tour"];
...
export interface Ctx {
  ...
  claimed: Set<string>;
  /**
   * Roadtrip station file id → station id in this account. A tour's anchor
   * cell names a station by the FILE's id; for a moved roadtrip that id means
   * nothing here, so the station sheet records where each one went.
   */
  stationsByFileId: Map<string, string>;
...
export function newCtx(opts: { userId: string; dryRun: boolean; mode: ImportMode }): Ctx {
  return {
    ...opts,
    byFileId: perKind(() => new Map<string, string>()),
    byLabel: perKind(() => new Map<string, Set<string>>()),
    claimed: new Set(),
    stationsByFileId: new Map(),
    touchedCruises: new Set(),
    wrote: false,
  };
}

function perKind<V>(make: () => V): Record<ParentKind, V> {
  return Object.fromEntries(PARENT_KINDS.map((k) => [k, make()])) as Record<ParentKind, V>;
}
