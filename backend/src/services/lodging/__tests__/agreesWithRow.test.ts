...
describe("a located hit must not contradict the row", () => {
  it("rejects the namesake in another country", () => {
    ...
    expect(
      agreesWithRow(row(), found({ city: "Rom", countryName: "Italien", lat: 41.9, lon: 12.5 }))
    ).toBe(false);
  });
  ...
  it("accepts a row that knows nothing about its place — that is what the tier is FOR", () => {
    ...
    expect(
      agreesWithRow(
        row({ city: null, country: null }),
        found({ city: "Shanghai", countryName: "China" })
      )
    ).toBe(true);
  });
  ...
  it("does not mistake spelling for disagreement", () => {
    expect(
      agreesWithRow(
        row({ city: "Zürich", country: "Schweiz" }),
        found({ city: "Zurich", countryName: "Schweiz" })
      )
    ).toBe(true);
    expect(agreesWithRow(row({ city: "Frankfurt" }), found({ city: "Frankfurt am Main" }))).toBe(
      true
    );
    expect(
      agreesWithRow(
        row({ city: "Rom", country: "Italien" }),
        found({ city: "Roma", countryName: "Italien" })
      )
    ).toBe(true);
  });
});
