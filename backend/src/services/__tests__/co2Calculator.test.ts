...
describe("calculateCo2Kg", () => {
  ...
  it("returns null when arrLat is null", () => {
    expect(
      calculateCo2Kg({ depLat: 50.033, depLon: 8.571, arrLat: null, arrLon: null, seatClass: null })
    ).toBeNull();
  });

  it("calculates economy CO₂ for short haul (FRA→MUC ~300km)", () => {
    const kg = calculateCo2Kg({
      depLat: 50.033,
      depLon: 8.571,
      arrLat: 48.354,
      arrLon: 11.786,
