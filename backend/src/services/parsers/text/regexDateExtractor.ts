 ...
 * the date.
 */
function timeOnPrecedingLine(
  source: string,
  matchIndex: number
): { h: string; min: string } | null {
...
export function extractAllTimePairs(
  ...
  const datumAbflugPattern =
    /Datum\s+der\s+Abreise\s+(\d{1,2})\.(\d{1,2})\.(\d{4})\s+Abflugzeit\s+(\d{1,2}):(\d{2})/gi;
  ...
  while ((match = germanPattern.exec(source)) !== null) {
    ...
    if (!month) {
      reject();
      continue;
    } // "2 von 1985" — not a month, not a date
    ...
    if (match[3].length === 2 && !match[0].includes(".")) {
      reject();
      continue;
    }
 ...
 */
export function extractLabeledDates(source: string): {
