...
const sourceTypes: Array<"email" | "boardingpass"> = [
  "email",
  "email",
  "email",
  "boardingpass",
  ...
  for (let i = 0; i < 18; i++) {
    ...
    switch (errorType) {
      case 0: // Flight number error
        originalResult = [
          {
            ...
            arrivalTime: new Date(
              createdAt.getTime() + route.duration * 60 * 60 * 1000
            ).toISOString(),
            pnr: pnr,
          },
        ];
        ...
        issues.push(
          `Flight 1: flightNumber mismatch (${originalResult[0].flightNumber} → ${flightNumber})`
        );
      ...
      case 1: {
        // Departure code error
        const wrongDep = airports.find(
          (a) => a.iata !== depAirport.iata && a.iata !== arrAirport.iata
        )!.iata;
        originalResult = [
          {
            flightNumber: flightNumber,
...
async function createDemoTrips(userId: string): Promise<void> {
  ...
  for (const def of tripDefs) {
    ...
    console.log(
      `   ✅ "${def.name}" — ${flightIds.length} flight(s)${def.pnr ? `, PNR ${def.pnr}` : " (manual)"}`
    );
    createdCount++;
  }
  ...
  try {
    ...
    if (existingFlights === 0) {
      ...
      for (let i = 0; i < flights.length; i += batchSize) {
        ...
        console.log(
          `   Created flights ${i + 1}-${Math.min(i + batchSize, flights.length)} of ${flights.length}`
        );
      }
      console.log("");
      const upcomingCount = flights.filter((f) => f.status === "scheduled").length;
      console.log(
        `✅ Created ${flights.length} flights (${upcomingCount} upcoming) across a weighted route mix`
      );
    }
