...
async function init() {
  ...
  try {
    ...
    try {
      ...
      logger.info({
        operation: "init_encryption_key_success",
    ...
    try {
      ...
      if (!fs.existsSync(backupPath)) {
        ...
        logger.info({
          operation: "init_backup_dir_created",
        ...
        logger.debug({
          operation: "init_backup_dir_exists",
          message: "Backup directory already exists",
          path: backupPath,
        });
      }
    } catch (error) {
    ...
    try {
      ...
      logger.info({
        operation: "init_migrations_success",
    ...
    logger.info({
      operation: "init_ensure_achievements",
      message: "Ensuring achievements are present",
    });
    try {
      ...
      logger.info({
        operation: "init_ensure_achievements_success",
        message: "Achievements ensured successfully",
      });
    } catch (error) {
    ...
    logger.debug({
      operation: "init_airport_seeding_info",
      message: "Airport seeding will start after first login",
    });
    ...
    if (shouldSeedDemo) {
      ...
      try {
        ...
        logger.info({
          operation: "init_demo_user_success",
      ...
      console.log(
        `6️⃣  Skipping demo user (DB already has ${userCount} user(s); set CREATE_DEMO_USER=true to force re-seed)\n`
      );
      logger.debug({
