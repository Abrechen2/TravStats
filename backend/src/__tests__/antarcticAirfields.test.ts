...
describe("isAntarcticAirfield", () => {
  ...
  it("does not claim types the normal filter already admits", () => {
    ...
    expect(isAntarcticAirfield(row({ type: "heliport", ident: "SAYO", gps_code: "SAYO" }))).toBe(
      false
    );
    expect(
...
describe("admitsAirport", () => {
  ...
  it("keeps closed commercial airports so historical flights stay loggable", () => {
    expect(admitsAirport(catalogueRow({ type: "closed", ident: "EDDI", iata_code: "THF" }))).toBe(
      true
    );
  });
  ...
  it("drops a row with no code at all", () => {
    expect(admitsAirport(catalogueRow({ ident: "", iata_code: "", gps_code: "" }))).toBe(false);
  });
});
