...
describe("normalizeAirportName", () => {
  it("strips the marker shape the source uses most", () => {
    expect(normalizeAirportName("[Duplicate] Beijing Xijiao Airport")).toBe(
  ...
  it("strips the malformed and hedged variants the source also carries", () => {
    ...
    expect(
      normalizeAirportName("(Misplaced duplicate?)Aeropuerto internacional Simón bolivar")
    ).toBe("Aeropuerto internacional Simón bolivar");
  });
...
describe("the catalogue repair migration carries the shared rule", () => {
  ...
  /**
   * Line endings are normalised, nothing else.
   *
   * Git checks this file out with CRLF on Windows, so a raw comparison against
   * the LF-joined string the builder returns fails on the owner's own machine --
   * it did, on the very first run after the merge. Normalising anything MORE
   * than the line endings (collapsing whitespace, say) would stop the guard
   * biting on the difference it exists to catch.
   */
  const lf = (text: string): string => text.replace(/\r\n/g, "\n");

  it("contains exactly the statement the shared rule generates", () => {
    const sql = lf(fs.readFileSync(migration, "utf-8"));
    expect(sql).toContain(lf(buildDuplicateMarkerCleanupSql("airports", "name")));
  });
});
