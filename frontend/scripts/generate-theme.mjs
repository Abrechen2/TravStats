...
export const TS_OUTPUT_PATH = resolve(here, "../src/theme/tokens.ts");
...
/**
 * The same tokens as a TypeScript module.
 *
 * A stylesheet is enough for anything the browser paints, but deck.gl and the
 * globe paint into a canvas: they take `[r, g, b]` arrays and cannot read a
 * custom property. Without this the map layers keep their own literals, which
 * is exactly how the map came to disagree with the legend it sits under. The
 * Companion generates `app/src/theme/tokens.ts` from the same file for the
 * same reason.
 *
 * Only the colours, and only as they are written in the JSON — no derived
 * shades. A layer that needs a tint computes it from one of these.
 */
export function buildThemeTs() {
  const tokens = JSON.parse(readFileSync(TOKENS_PATH, "utf8"));
  const strip = (group) =>
    Object.fromEntries(Object.entries(group).filter(([key]) => !isMeta(key)));

  const payload = {
    color: strip(tokens.color),
    domainColor: strip(tokens.domainColor),
    statusColor: strip(tokens.statusColor),
    tierColor: strip(tokens.tierColor),
    chartColors: tokens.chartColors,
    chartMutedBar: tokens.chartMutedBar,
    listColor: strip(tokens.listColor.palette),
    map: strip(tokens.map),
  };
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  ...
  writeFileSync(TS_OUTPUT_PATH, buildThemeTs(), "utf8");
  console.log("tokens.css + tokens.ts written from design/tokens.json");
}
