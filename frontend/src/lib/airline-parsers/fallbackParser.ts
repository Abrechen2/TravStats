...
export class FallbackParser implements BoardingPassParser {
  ...
  parse(barcodeData: string): BoardingPassData | null {
    try {
      logger.debug(
        "[Fallback Parser] Attempting fallback parsing on:",
      ...
      if (likelyAirports.length >= 2) {
        ...
        logger.debug(
          "[Fallback Parser] Airports found:",
          extracted.departureAirport,
          "→",
          extracted.arrivalAirport
        );
      } else if (airports.length >= 2) {
        extracted.departureAirport = airports[0];
        extracted.arrivalAirport = airports[1];
        logger.debug(
          "[Fallback Parser] Airports found (fallback):",
          extracted.departureAirport,
          "→",
          extracted.arrivalAirport
        );
      ...
      if (flights.length > 0) {
        ...
        logger.debug(
          "[Fallback Parser] Flight found:",
          extracted.operatingCarrierDesignator + extracted.flightNumber
        );
      }
