...
describe("dayShift", () => {
  it("is 0 for a same-day flight", () => {
    expect(
      dayShift("2026-08-15T08:20:00Z", "2026-08-15T10:05:00Z", "Europe/Berlin", "Europe/Copenhagen")
    ).toBe(0);
  });
  ...
  it("is 1 for an overnight eastbound flight", () => {
    ...
    expect(
      dayShift("2026-05-02T19:40:00Z", "2026-05-03T02:45:00Z", "Europe/Berlin", "Asia/Dubai")
    ).toBe(1);
  });
  ...
  it("can be negative for westbound across midnight the other way", () => {
    ...
    expect(
      dayShift("2026-05-01T16:00:00Z", "2026-05-02T00:00:00Z", "Asia/Tokyo", "America/Los_Angeles")
    ).toBe(-1);
  });
