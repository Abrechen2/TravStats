...
describe("deriveStayMembership", () => {
  ...
  it("breaks a createdAt tie on the lexicographically smaller id, regardless of array order", () => {
    ...
    expect(
      deriveStayMembership({ ...base, lodgingChainId: 7, memberships: [tiedA, tiedB] }).membershipId
    ).toBe("m-tied-a");
    expect(
      deriveStayMembership({ ...base, lodgingChainId: 7, memberships: [tiedB, tiedA] }).membershipId
    ).toBe("m-tied-a");
  });
