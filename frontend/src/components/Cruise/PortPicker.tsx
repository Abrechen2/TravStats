...
export function PortPicker({ value, onChange, label }: Props): JSX.Element {
  ...
  return (
    <div className="relative">
      ...
      {/* Shared error slot: covers both the geocoded-select failure (no
          "add custom" sub-form open) and the custom-port save failure below —
          previously this only rendered inside the `showAdd` block, so a
          `handleSelectGeocoded` failure set `error` but never displayed it. */}
      {error !== null && !showAdd && <p className="mt-1 text-xs text-[var(--danger)]">{error}</p>}
      {query.length >= 2 &&
        query !== value?.name &&
        !exactMatch &&
        geocoded.length === 0 &&
