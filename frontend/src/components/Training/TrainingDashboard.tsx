...
export default function TrainingDashboard({
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const runningJobs = jobsResult.jobs.filter(
  ...
  const handleFilterChange = useCallback(
    (newFilters: { status?: string; type?: string; tags?: string[]; search?: string }) => {
  ...
  const getProgressFromLogs = (
    logs: JobLogs | undefined,
    startedAt?: string | Date
  ): { phase: string; progress: number; estimatedTimeRemaining?: number } => {
    if (!logs) {
    ...
    if (
      logFileContent.includes("training completed successfully") ||
    ...
    // Check for saving model
      ...
      logFileContent.includes("saving model") ||
    ...
    // Check for model preparation
      ...
      logFileContent.includes("preparing model") ||
    ...
    // Check for loading model
      ...
      logFileContent.includes("loading tokenizer") ||
    ...
    // Check for training start
      ...
      logMessages.some((m) => m.includes("starting lora training")) ||
  ...
  const handleTriggerTraining = async () => {
    ...
    try {
      ...
      addToast(
        "error",
        axiosError.response?.data?.message || t("training:errors.startTrainingFailed")
      );
    } finally {
      setTriggering(false);
    }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await Promise.all(Array.from(selectedItems).map((id) => trainingApi.deleteTrainingData(id)));
      setSelectedItems(new Set());
      await loadData();
    } catch (error: unknown) {
  ...
  const handleCancelTraining = async () => {
    ...
    try {
      ...
      const errorMessage =
        ...
        axiosError.message ||
        t("training:errors.cancelTrainingFailed");
