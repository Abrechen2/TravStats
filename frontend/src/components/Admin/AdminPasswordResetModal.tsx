...
export default function AdminPasswordResetModal({
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={`${t("admin:users.resetPasswordModal.title")} — ${username}`}
      maxWidth={448}
      closeLabel={t("admin:users.resetPasswordModal.close")}
    >
      {/* Tabs */}
      ...
      {activeTab === "set" && (
        <div className="space-y-4">
          ...
          {setSuccess ? (
            <p className="text-sm text-green-400">{t("admin:users.resetPasswordModal.success")}</p>
          ) : (
