 ...
 *
 * NOTE on the "abandoned edit" test below: an earlier round of this task
 * added a "settled unresolved" hint so the user could see when a typed
 * airport edit didn't resolve. That hint was reviewed, fixed, and reviewed
 * again across four rounds — mid-typing false positives, then mid-dropdown-
 * pick false positives caused by the browser's own blur-before-click
 * ordering, then a deferred check needing cancellation on refocus, at which
 * point proving the fix required `vi.useFakeTimers()` state updates this
 * test environment could not reliably observe even for the already-passing
 * case. It was removed rather than pushed to a fifth round (see
 * RouteFields.tsx's docblock for the full record) — this test now only
 * pins the underlying, never-actually-broken data behaviour: an abandoned
 * edit is silently DROPPED (not corrupted), which the UI gap this leaves
 * behind is a recorded, known, low-severity limitation.
 */
...
// here. The stub reproduces its value/onChange contract with one button per
// action:
...
describe("FlightEditModal route editing (Task 4)", () => {
  ...
  it("changing an airport submits the newly picked one, not the flight's original", async () => {
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(<FlightEditModal flight={FLIGHT} isOpen onClose={() => {}} onSave={onSave} />);
    await waitForHydration();
    ...
    const [, payload] = onSave.mock.calls[onSave.mock.calls.length - 1];
    expect(payload.departure).toMatchObject({ iata: "LHR", lat: 51.5, lon: -0.45 });
  });

  // An abandoned typed edit (AirportAutocomplete's own onChange(null) when
  // the typed text doesn't resolve) is a known, deliberately-not-fixed UI
  // gap — see this file's header and RouteFields.tsx's docblock. What
  // matters is that it stays a UI-only gap: no corruption, no wrong airport
  // submitted, just the field omitted from the payload so the server keeps
  // whatever the flight already had stored.
  it("omits the field (not corrupts it) when a typed edit never resolves to a real airport", async () => {
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(<FlightEditModal flight={FLIGHT} isOpen onClose={() => {}} onSave={onSave} />);
    await waitForHydration();
