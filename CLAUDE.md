...
- **Cruise migrations** — the initial cruise migration
  ...
  `prisma migrate dev`-generated. Reason: the existing schema.prisma has
  pre-existing drift vs. the migration history (NOT-NULL flips on
  `flights.has_live_tracking` + `user_settings.historical_enrichment_*`,
  plus unrelated DROP INDEX reconciliations) that `prisma migrate dev`
  would bundle into any new migration and break prod on deploy. The
  drift fix must be resolved separately before any non-trivial
  migration can be generated by Prisma again — see the prod-drift audit
  note in auto-memory.
- **Cruise parser is a 501 stub** — the parser routes accept
  `domain=cruise` and return `501 PARSER_DOMAIN_NOT_IMPLEMENTED` with a
  clean message. Real AIDA / TUI templates + LLM cruise prompt are V2
  and require sample booking emails to test against.
...
This project is indexed by GitNexus as **TravStats** (3862 symbols, 10103 relationships, 283 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
